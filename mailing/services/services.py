...
from mailing.models import LogMessage
from mailing.models import SetMessage, Client
...
def get_count_mailing():
    ...
    return SetMessage.objects.count()


def get_active_mailing():
    """Возвращает количество активных рассылок"""
    return SetMessage.objects.filter(status='START').count()
